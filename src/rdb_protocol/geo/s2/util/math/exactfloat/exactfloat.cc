...
inline static uint64 BN_ext_get_uint64(const BIGNUM* bn) {
  ...
  const dirtyhack_bignum_st* bnhack = reinterpret_cast<const dirtyhack_bignum_st*>(bn);
  if (bnhack->top == 0) return 0;
  if (bnhack->top == 1) return BN_get_word(bn);
  DCHECK_EQ(bn->top, 2);
  return (static_cast<uint64>(bnhack->d[1]) << 32) + bnhack->d[0];
#endif
}
